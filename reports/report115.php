				...
				read_json_rows('report115',orders_data,function(orders)
	            {
		          	...
		          	branches.forEach(function(branch)
		            {
						...
						branch.cod_pending=branch.total_cod-branch.cod_collected;
		                  ...
		                  rowsHTML+="</a></td>";
						  rowsHTML+="<td data-th='In-transit'><a onclick=\"report115_popup_action('"+branch.name+"','transit');\">";
					...
					vExport.export_buttons({file:'Branch Stock Report',report_id:'report115',action:'static'});
		...
		function report115_popup_action(branch,type)
		{
			...
			switch(type)
			{
				case 'total':status_object={index:"status"};
							break;
				case 'trnasit':status_object={index:"status",exact:'in-transit'};
							break;
