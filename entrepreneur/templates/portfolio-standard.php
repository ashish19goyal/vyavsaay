...
$args = array(
    ...
    if(isset($filter_bar[0]) && $filter_bar[0] == 1) {
        ...
        echo '<span>'. __('Sort:', THEMO_TEXT_DOMAIN) . '</span><a href="#" data-filter="*" class="current">All</a>'. $project_type_links;
        echo '</div>';
    ...
    echo '<div id="portfolio-row" class="portfolio-row row '.$key .' '. $col_row_class.'">';
		if (!$loop->have_posts()) {
            ...
            _e('Sorry, no results were found.', THEMO_TEXT_DOMAIN);
            echo '</div>';
            get_search_form();
        }
    ...
    <div class="row">
		<?php if ($loop->max_num_pages > 1) : ?>
            <nav class="post-nav">
                <ul class="pager">
                    <li class="previous"><?php next_posts_link(__('&larr; Previous', THEMO_TEXT_DOMAIN), $loop->max_num_pages); ?></li>
                    <li class="next"><?php previous_posts_link(__('Next &rarr;', THEMO_TEXT_DOMAIN), $loop->max_num_pages); ?></li>
                </ul>
            </nav>
        <?php endif; ?>
	</div>
