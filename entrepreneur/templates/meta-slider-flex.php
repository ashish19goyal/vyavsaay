...
if($show == 1) { // Slider Enabled
	...
	if(isset($down_arrow) && $down_arrow == 1 && isset($down_arrow_anchor) && $down_arrow_anchor > ""){
		...
		$down_arrow_markup = '<a href="'.$down_arrow_anchor.'" target="_self" class="slider-scroll-down th-icon th-i-down"></a>';
	}
		...
		while ($loop->have_posts()) {
			...
			echo wp_kses_post($narrow_format_open);
				...
				*/
				...
				// Button
				$meta_box_show_button = $element['__show_button'][0]; // get button state
				if($meta_box_show_button == 1){
					$page_header_button_text = $element['__button_text'][0]; // button text
					$page_header_button_link = $element['__button_link'][0]; // button link
					$page_header_button_style = $element['__button_style'][0]; // button style
