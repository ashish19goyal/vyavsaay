...
if(isset($options_array) && !empty( $options_array )){
	...
	$background_show = get_post_meta($post->ID, '__show_background', true );
	$background = array(); //get_post_meta($post->ID, $key.'_background', true );
	$background['background-image'] = get_post_meta($post->ID, '__background_image', true );
	$background['background-color'] = get_post_meta($post->ID, '__background_color', true );
	$background['background-repeat'] = get_post_meta($post->ID, '__background_repeat', true );
	$background['background-position'] = get_post_meta($post->ID, '__background_position', true );
	$background['background-attachment'] = get_post_meta($post->ID, '__background_attachment', true );
	$background['background-size'] = get_post_meta($post->ID, '__background_size', true );

	$background_parallax_show = get_post_meta($post->ID, '__parallax', true );
	$text_contrast = get_post_meta($post->ID, '__text_contrast', true );
	$padding_show = get_post_meta($post->ID, '__padding', true );
	$top_padding = get_post_meta($post->ID, '__top_padding', true );
	$bottom_padding = get_post_meta($post->ID, '__bottom_padding', true );
}else{
	$background_show = get_post_meta($post->ID, $key.'_show_background', true );
	$background = array(); //get_post_meta($post->ID, $key.'_background', true );
	$background['background-image'] = get_post_meta($post->ID, $key.'_background_image', true );
	$background['background-color'] = get_post_meta($post->ID, $key.'_background_color', true );
	$background['background-repeat'] = get_post_meta($post->ID, $key.'_background_repeat', true );
	$background['background-position'] = get_post_meta($post->ID, $key.'_background_position', true );
	$background['background-attachment'] = get_post_meta($post->ID, $key.'_background_attachment', true );
	$background['background-size'] = get_post_meta($post->ID, $key.'_background_size', true );
