...
// @return_shortocde = return or print
//-----------------------------------------------------
function themo_do_shortocde_button($postid='',$key='',$return_shortcode=false, $extra_classes=''){
	...
	if ( function_exists( 'ot_get_option' ) ) {
		...
		if ($full_width == 'off'){
			...
			if($background_settings > "" && $is_full_width && $backstretch == 'on'){
				...
				$body_backstretch_js .= sanitize_text_field("$background_js\n");
				$body_backstretch_js .= "});\n";
...
function themo_ot_meta_box_post_format_link($array,$pages) {
	...
	$array['fields'] = array(
		...
		array(
			...
			'label'       => __( 'Link behaviour on the portfolio homepage', THEMO_TEXT_DOMAIN ),
...
function themo_register_required_plugins() {
	...
	$plugins = array(
		...
		array(
			...
			'version'            => '1.5.3', // E.g. 1.0.0. If set, the active plugin must be this version or higher.
			'force_activation'   => false, // If true, plugin is activated upon theme activation and cannot be deactivated until theme switch.
			'force_deactivation' => false, // If true, plugin is deactivated upon theme switch, useful for theme-specific plugins.
			'external_url'       => '', // If set, overrides default API URL and points to an external URL.
		),
		...
		array(
			'name'      => 'Smart Website Tools by AddThis',
			'slug'      => 'addthis',
			'required'  => false,
		),
		array(
...
function themo_import_end(){
	...
	$page_names = array('Stylist Home','Trainer Home','Physician Home','Coach Home','Consultant Home','Contractor Home','Entrepreneur Home','Careers','Showcase'); // list of all possible page names.
	...
	$term_names = array('stylist-home','trainer-home','physician-home','coach-home','consultant-home','contractor-home','entrepreneur-home'); // list of all possible term names.
	...
	$port_term_names = array('stylist-portfolio-home','trainer-portfolio-home','physician-portfolio-home','coach-portfolio-home','consultant-portfolio-home','contractor-portfolio-home','entrepreneur-portfolio-home'); // list of all possible term names.
	...
	printf(__('<a href="%2$s">Back to Theme Activation 2</a> (not completed yet) | <a href="%1$s"><small><em>dismiss</em></small></a>', THEMO_TEXT_DOMAIN), '?themo_activation_nag_ignore=0',admin_url('themes.php?page=theme_activation_options'));
	echo "</div>";*/
}
