    ...
    <div class="wrap">
      ...
      <?php _e('These settings are optional and should usually be used only on a fresh installation', THEMO_TEXT_DOMAIN); ?>
        ...
        <div class="theme-activation-options theme-activation-step-one">
            <h2><?php _e('1 - Plugins', THEMO_TEXT_DOMAIN); ?></h2>
            <?php
        ...
        <div class="theme-activation-options theme-activation-step-two">
            <h2><?php _e('2 - Demo Content', THEMO_TEXT_DOMAIN); ?></h2>
            ...
            <?php }else{ ?>
                <p class="dashicons-before dashicons-plus"><?php printf(__('<strong>%1$sImport Demo Content%2$s</strong>  - "%3$s", included in theme zip.', THEMO_TEXT_DOMAIN), '<a href="'.get_admin_url().'admin.php?import=wordpress'.'">','</a>',wp_get_theme().'-demo-content.xml'); ?></p>
                <p style="padding:10px; background-color:#e7e7e7; max-width:600px" class="dashicons-before dashicons-clock"><?php printf(__('Importing  takes time. %sLearn more.%s', THEMO_TEXT_DOMAIN), '<a href="http://docs.themovation.com/entrepreneur/#democontent'.'" target="_blank">','</a></em>'); ?></p>
            <?php }
            ?>
            ...
            <?php }else{ ?>
                <p class="dashicons-before dashicons-plus"><?php printf(__('<strong>%1$sImport Widget Content%2$s </strong> - "%3$s", includced in theme zip.', THEMO_TEXT_DOMAIN), '<a href="'.get_admin_url().'tools.php?page=widget-importer-exporter'.'">','</a>',wp_get_theme().'-demo-widget-content.wie'); ?></p>
            <?php } ?>
            ...
            <?php }else{ ?>
                <p class="dashicons-before dashicons-plus"><?php printf(__('<strong>%1$sImport Forms%2$s</strong> - "%3$s", includced in theme zip.', THEMO_TEXT_DOMAIN), '<a href="'.get_admin_url().'admin.php?page=formidable-import'.'">','</a>',wp_get_theme().'-formidable-custom-forms.xml'); ?></p>
            <?php } ?>
        ...
        <div class="theme-activation-options theme-activation-step-three">
            <h2><?php _e('3 - Settings', THEMO_TEXT_DOMAIN); ?></h2>
            ...
            <?php }else{ ?>
                ...
                <form method="post" action="options.php">
                    ...
                    <table class="form-table">
                        ...
                        <?php
                        $page_names = array('Trainer Home','Physician Home','Coach Home','Consultant Home','Contractor Home','Entrepreneur Home','Stylist Home'); // list of all possible page names.
                        ...
                        <?php }else{ ?>
                            ...
                            <tr valign="top"><th scope="row"><?php _e('Create Front page?', THEMO_TEXT_DOMAIN); ?></th>
                                <td>
                                    <fieldset>
                                        <legend class="screen-reader-text"><span><?php _e('Create Front page?', THEMO_TEXT_DOMAIN); ?></span></legend>
                                        ...
                                        <p class="description"><?php printf(__('Create a page called Home and set it to be the Front page.', THEMO_TEXT_DOMAIN)); ?></p>
                                    </fieldset>
                                </td>
                            </tr>
                        <?php } ?>
                        <?php if(get_page_by_title('blog')){ ?>
                            <tr valign="top"><th scope="row"><?php _e('Set Post page?', THEMO_TEXT_DOMAIN); ?></th>
                                <td>
                                    <fieldset>
                                        <legend class="screen-reader-text"><span><?php _e('Set a Post page to be your default blog.', THEMO_TEXT_DOMAIN); ?></span></legend>
                                        <?php
                                        $args = array(
                                            'sort_order' => 'ASC',
                                            'sort_column' => 'post_title',
                            ...
                            <?php echo wp_kses_post($option_output); ?>
                                        ...
                                        <p class="description"><?php printf(__('Set a page it to be the Post page (default blog)', THEMO_TEXT_DOMAIN)); ?></p>
                                    </fieldset>
                                </td>
                            </tr>
                        <?php }else{ ?>

                            <tr valign="top"><th scope="row"><?php _e('Create Post page?', THEMO_TEXT_DOMAIN); ?></th>
                                <td>
                                    <fieldset>
                                        <legend class="screen-reader-text"><span><?php _e('Create Post page?', THEMO_TEXT_DOMAIN); ?></span></legend>
                                        <select name="roots_theme_activation_options[create_post_page]" id="create_post_page">
                                            <option selected="selected" value="true"><?php echo _e('Yes', THEMO_TEXT_DOMAIN); ?></option>
                                            <option value="false"><?php echo _e('No', THEMO_TEXT_DOMAIN); ?></option>
                                        </select>
                                        <p class="description"><?php printf(__('Create a page called Blog and set it to be the Post page', THEMO_TEXT_DOMAIN)); ?></p>
                                    </fieldset>
                                </td>
                            </tr>
                        <?php } ?>
...
function themo_activation_admin_notice() {
    ...
    if ( ! get_user_meta($user_id, 'themo_activation_ignore_notice') && ($pagenow == 'themes.php' || $pagenow == 'tools.php' || $pagenow == 'admin.php' || $pagenow == 'plugins.php')) {
        ...
        printf(__('<a href="%2$s">Back to Theme Activation</a> (not completed yet) | <a href="%1$s"><small><em>dismiss</em></small></a>', THEMO_TEXT_DOMAIN), '?themo_activation_nag_ignore=0',admin_url('themes.php?page=theme_activation_options'));
        echo "</div>";
    }
}
