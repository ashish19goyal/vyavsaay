...
function cron4()
{
	...
	// {
		...
		$domain="dentalarch";
		$email_instance=emailSES::getInstance($domain);
		...
		for($j=0;$j<count($app_result);$j++)
		{
			...
			if($email!="" && $email!=null)
			{
				...
				$email_message="Your appointment at $bt is confirmed for $schedule_format Dont forget to visit.<br><br>Regards,<br>$bt<br>$baddress<br>$bphone";
				$r_array=array(array('email'=>$email));
