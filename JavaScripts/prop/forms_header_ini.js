...
function form299_header_ini()
{
	...
	$(add_image_button).on('click',function () 
	{
		...
		{
			var image_elem="<li class='newsletter_component' id='form299_image_"+pic_id+"' data-name='"+name+"' data-id='"+pic_id+"' data-url='"+url+"'><div style='float:left;width:80%'>"+name+"</div><i style='float:right;width:20%;' class='fa fa-times' onclick=\"form299_delete_image('"+pic_id+"');\"></i></li>";
	...
	$("#form299_navigation.sortable" ).sortable
	({
		axis: "y",
		...
		update: function(event, ui) 
		{
			...
			new_div.setAttribute('class','print_section');
			new_div.setAttribute('id','form299_section');
			$('#form299').append(new_div);
		}
	});
