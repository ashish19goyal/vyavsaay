<div id='form306' class='tab-pane portlet box green-meadow'>	   
	<div class="portlet-title">
		...
		<div class="actions">
      	<a class='btn btn-default btn-sm' id='form306_csv'><i class='fa fa-file-excel-o'></i> Save as CSV</a>
      	<a class='btn btn-default btn-sm' id='form306_pdf'><i class='fa fa-file-pdf-o'></i> Save as PDF</a>
      	<a class='btn btn-default btn-sm' id='form306_print'><i class='fa fa-print'></i> Print</a>
      </div>	
	</div>
					...
					var rowsHTML="<tr>";
						rowsHTML+="<form id='form306_"+result.id+"'></form>";
							...
							rowsHTML+="<td data-th='Details'>";
								rowsHTML+="<button type='button' class='btn default yellow-stripe' form='form306_"+result.id+"' onclick=\"modal184_action('"+result.id+"');\">Box Content</button>";							
								rowsHTML+="<button type='button' class='btn default purple-stripe' form='form306_"+result.id+"' onclick=\"modal185_action('"+result.id+"');\">Function</button>";							
							rowsHTML+="</td>";
							rowsHTML+="<td data-th='Action'>";
								...
								rowsHTML+="<button class='btn red' form='form306_"+result.id+"' title='Delete' onclick='form306_delete_item($(this));'><i class='fa fa-trash'></i></button>";
							rowsHTML+="</td>";			
					rowsHTML+="</tr>";
			...
			if(is_create_access('form306'))
			{
				...
				var rowsHTML="<tr>";
						rowsHTML+="<form id='form306_"+id+"'></form>";
							...
							rowsHTML+="</td>";
							rowsHTML+="<td data-th='Details'>";
								rowsHTML+="<button type='button' class='btn default yellow-stripe' form='form306_"+id+"' onclick=\"modal184_action('"+id+"');\">Box Content</button>";							
								rowsHTML+="<button type='button' class='btn default purple-stripe' form='form306_"+id+"' onclick=\"modal185_action('"+id+"');\">Function</button>";							
							rowsHTML+="</td>";
							rowsHTML+="<td data-th='Action'>";
								...
								rowsHTML+="<button class='btn red' form='form306_"+id+"' title='Delete' onclick='$(this).parent().parent().remove();'><i class='fa fa-trash'></i></button>";
							rowsHTML+="</td>";			
					rowsHTML+="</tr>";
		...
		function form306_create_item(form)
		{
			if(is_create_access('form306'))
			{
				...
				var data_json={data_store:'system_popboxes',
	 				...
	 				log_data:{title:'Added',notes:'Popup box '+name,link_to:'form306'}};
			...
			if(is_update_access('form306'))
			{
				...
				var data_json={data_store:'system_popboxes',
	 				...
	 				data:[{index:'id',value:data_id},
	 					...
	 					{index:'status',value:status},
	 					{index:'last_updated',value:last_updated}],
	 				log_data:{title:'Updated',notes:'Popup box '+name,link_to:'form306'}};
			...
			if(is_delete_access('form306'))
			{
				modal115_action(function()
				{
					...
					var data_json={data_store:'system_popboxes',
 							...
 							log:'yes',
 							log_data:{title:"Deleted",notes:"Popup box "+name,link_to:"form306"}};
